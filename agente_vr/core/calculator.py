...
class VRCalculator:
    ...
    def calcular_vr_completo(
        ...
        try:
            ...
            for colaborador in colaboradores_elegiveis:
                ...
                # Atualizar estatísticas parciais (mantém compatibilidade)
            ...
            # Finalizar estatísticas completas para o resumo final
            fim_processamento = datetime.now()
            self.estatisticas['tempo_processamento'] = fim_processamento - inicio_processamento
            self.estatisticas['arquivos_processados'] = self._contar_arquivos_processados()
            self.estatisticas['colaboradores_total'] = resumo_financeiro.total_colaboradores
            self.estatisticas['colaboradores_elegiveis'] = self._contar_colaboradores_elegiveis(colaboradores_elegiveis)
            self.estatisticas['colaboradores_excluidos'] = self._contar_colaboradores_excluidos(colaboradores_elegiveis)
            self.estatisticas['valor_total_calculado'] = resumo_financeiro.valor_total_geral
            self.estatisticas['valor_empresa'] = resumo_financeiro.valor_total_empresa
            self.estatisticas['valor_colaboradores'] = resumo_financeiro.valor_total_colaboradores
            ...
            raise
    def _contar_arquivos_processados(self) -> int:
        # Tenta contar arquivos processados se houver referência aos dados carregados
        try:
            from core.data_loader import DataLoader
            # Supondo que DataLoader armazene os nomes dos arquivos carregados
            if hasattr(self, 'data_loader') and hasattr(self.data_loader, 'arquivos_carregados'):
                return len(self.data_loader.arquivos_carregados)
        except Exception:
            pass
        return 0

    def _contar_colaboradores_elegiveis(self, colaboradores_elegiveis) -> int:
        # Considera todos como elegíveis se não houver status
        try:
            return len([c for c in colaboradores_elegiveis if getattr(c, 'status', None) != 'EXCLUIDO'])
        except Exception:
            return len(colaboradores_elegiveis)

    def _contar_colaboradores_excluidos(self, colaboradores_elegiveis) -> int:
        try:
            return len([c for c in colaboradores_elegiveis if getattr(c, 'status', None) == 'EXCLUIDO'])
        except Exception:
            return 0
    ...
    def _calcular_vr_individual(
        ...
        dados_colaborador = dados_auxiliares.get(str(matricula), {})
        ...
        valores: Dict[str, Decimal] = self._calcular_valores_financeiros(
    ...
    def _gerar_resumo_financeiro(self, calculos: List[CalculoVRDetalhado]) -> ResumoFinanceiroVR:
        ...
        total_dias_pagos = sum(int(c.dias_vr_pagos) for c in calculos)
        valor_total_geral: Decimal = sum((Decimal(str(c.valor_total_vr)) for c in calculos), Decimal('0'))
        valor_total_empresa: Decimal = sum((Decimal(str(c.valor_custo_empresa)) for c in calculos), Decimal('0'))
        valor_total_colaboradores: Decimal = sum((Decimal(str(c.valor_desconto_colaborador)) for c in calculos), Decimal('0'))
        # Garantir que os tipos sejam sempre Decimal, nunca Literal[0]
        valor_total_geral = Decimal(valor_total_geral)
        valor_total_empresa = Decimal(valor_total_empresa)
        valor_total_colaboradores = Decimal(valor_total_colaboradores)
        ...
        # Métricas
        valores = [Decimal(str(c.valor_total_vr)) for c in calculos]
        valor_medio = valor_total_geral / Decimal(str(total_colaboradores)) if total_colaboradores > 0 else Decimal('0')
        valor_mediano = self._calcular_mediana_decimal(valores)
        dias_medio = total_dias_pagos / total_colaboradores if total_colaboradores > 0 else 0.0
    ...
    def _preparar_dados_auxiliares(self, dados_adicionais: Optional[Dict[str, pd.DataFrame]]) -> Dict[str, Dict[str, Any]]:
        ...
        dados_aux: Dict[str, Dict[str, Any]] = {}
        if dados_adicionais:
    ...
    def _agrupar_por_estado(self, calculos: List[CalculoVRDetalhado]) -> Dict[str, Dict[str, Any]]:
        ...
        for calculo in calculos:
            ...
            resumo[estado]['dias_totais'] += int(calculo.dias_vr_pagos)
            resumo[estado]['valor_total'] += Decimal(str(calculo.valor_total_vr))
            resumo[estado]['valor_empresa'] += Decimal(str(calculo.valor_custo_empresa))
            resumo[estado]['valor_colaboradores'] += Decimal(str(calculo.valor_desconto_colaborador))
        return resumo
    ...
    def _calcular_distribuicao_dias(self, calculos: List[CalculoVRDetalhado]) -> Dict[int, int]:
        ...
        for calculo in calculos:
            dias = int(calculo.dias_vr_pagos)
    ...
    def _atualizar_estatisticas_calculo(self, calculo: CalculoVRDetalhado) -> None:
        ...
        # Garantir tipos corretos antes de somar
        total_calculados_raw = self.estatisticas.get('total_calculados', 0)
        total_calculados: int
        if type(total_calculados_raw) is int:
            total_calculados = total_calculados_raw
        elif type(total_calculados_raw) is float:
            total_calculados = int(total_calculados_raw)
        elif type(total_calculados_raw) is str and total_calculados_raw.isdigit():
            total_calculados = int(total_calculados_raw)
        else:
            total_calculados = 0

        valor_total_calculado_raw = self.estatisticas.get('valor_total_calculado', '0')
        try:
            valor_total_calculado = Decimal(str(valor_total_calculado_raw))
        except Exception:
            valor_total_calculado = Decimal('0')

        inconsistencias_calculo_raw = self.estatisticas.get('inconsistencias_calculo', 0)
        inconsistencias_calculo: int
        if type(inconsistencias_calculo_raw) is int:
            inconsistencias_calculo = inconsistencias_calculo_raw
        elif type(inconsistencias_calculo_raw) is float:
            inconsistencias_calculo = int(inconsistencias_calculo_raw)
        elif type(inconsistencias_calculo_raw) is str and inconsistencias_calculo_raw.isdigit():
            inconsistencias_calculo = int(inconsistencias_calculo_raw)
        else:
            inconsistencias_calculo = 0

        total_calculados += 1
        valor_total_calculado += Decimal(str(calculo.valor_total_vr))
        if not calculo.calculo_validado:
            inconsistencias_calculo += 1
        self.estatisticas['total_calculados'] = total_calculados
        self.estatisticas['valor_total_calculado'] = valor_total_calculado
        self.estatisticas['inconsistencias_calculo'] = inconsistencias_calculo
    ...
    def _log_resumo_calculos(self, calculos: List[CalculoVRDetalhado], resumo: ResumoFinanceiroVR) -> None:
        ...
        self.logger.info(f"   Colaboradores calculados: {int(len(calculos))}")
        self.logger.info(f"   Total de dias pagos: {int(resumo.total_dias_pagos) if not isinstance(resumo.total_dias_pagos, int) else resumo.total_dias_pagos}")
        self.logger.info(f"   Valor total: R$ {float(resumo.valor_total_geral):,.2f}")
        self.logger.info(f"   Custo empresa: R$ {float(resumo.valor_total_empresa):,.2f}")
        self.logger.info(f"   Desconto colaboradores: R$ {float(resumo.valor_total_colaboradores):,.2f}")
        self.logger.info(f"   Valor médio por colaborador: R$ {float(resumo.valor_medio_por_colaborador):,.2f}")
