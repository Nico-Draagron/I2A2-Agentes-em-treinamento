...
class AssistenteGemini:
    ...
    def analisar_inconsistencias(self, dados: Dict[str, pd.DataFrame]) -> Dict:
        """Analisa inconsistências nos dados usando Gemini"""
        if not self.gemini_disponivel:
            return self._analise_offline(dados)
        try:
            # Iterar sobre todos os DataFrames e gerar um resumo simples
            resumos = {}
            for nome, df in dados.items():
                if hasattr(df, 'columns'):
                    ...
                    resumos[nome] = 'Não é DataFrame'
            # Aqui você pode adaptar para enviar esse resumo para o Gemini se desejar
...
class ProcessadorVR:
    ...
    def __init__(self, config: ConfiguracaoVR):
        self.config = config
        self.logger = logging.getLogger(__name__)
        self.gemini = AssistenteGemini(config)
    ...
    def _carregar_dados(self) -> bool:
        ...
        if self.usar_modulos_existentes:
            # Usar seu DataReader
            try:
